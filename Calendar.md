...
The calendar can be navigated by the buttons above it. The two buttons to the right switch between a month and week view. When switching to the week view the first week of the month will be shown. In the month view each session is reduced to a single entry, while in the week view the session entry is expanded to span the reserved time.

The arrows switch to the respective week/month before or after. The today button sets the calendar to the month or week containing the current day.
...
After clicking on a session, a pop up with further information on it shows up. Those include the exact duration of the session, the location, the experimenters, the actual numbers of participants signed up, required and reserve, setup and tear down time, links to the participants, as well as the options page of the session.
