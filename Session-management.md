...
* **Envelope:** Signals that this user has been sent an invitation email. *(Note: This status might be cleared when using "Invite all users" bulk functions, so the icon may sometimes be missing even if invited).*
* **Flag:** Indicates that a reminder email has been sent to the user.
* **Exclamation Mark:** Represents a potential duplicate account:
...
* **Trash Can Icon:** Indicates that the user is permanently deleted (personal data removed, but record kept for statistics/accounting).
