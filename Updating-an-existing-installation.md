...
* [7. Changing the Update Path (Switching from Upstream to Custom Fork or Branch)](#7-changing-the-update-path-switching-from-upstream-to-custom-fork-or-branch)
...
## 7. Changing the Update Path (Switching from Upstream to Custom Fork or Branch)

If your installation previously pulled updates from the official upstream repository (`ghcr.io/wiso-forschungslabor/hroot:latest` / `wiso-forschungslabor/hroot` on `master`) and you now want to switch to a **custom fork** (or switch between different branches such as `master` $\leftrightarrow$ `staging`), follow this step-by-step migration procedure.

### Why Switch the Update Path?
* Your laboratory created a custom fork (e.g. `https://github.com/<your-account>/hroot`) to implement institutional customizations, custom themes, or specialized integrations.
* You built your own Docker images via GitHub Actions and want your running server (and the In-App Update button) to follow your fork's release cycle instead of the official upstream repository.
* You want to test a pre-release branch (e.g., `feature/custom-auth` or `staging`) before rolling it out to production.

---

### Step-by-Step Procedure to Change the Update Path

#### Step 1: Ensure Your Fork Image is Built & Host is Authenticated
1. In your GitHub fork repository, ensure the Docker build workflow has executed successfully (via GitHub Actions) and pushed an image to `ghcr.io/<your-account>/hroot:<tag>`.
2. Confirm the package visibility in GitHub is set to **Private** (mandatory per license).
3. On your production server host, ensure Docker is authenticated with GitHub Container Registry using a Personal Access Token (PAT) with `read:packages` permissions:
   ```bash
   echo "<YOUR_GITHUB_PAT>" | docker login ghcr.io -u <YOUR_GITHUB_USERNAME> --password-stdin
   ```

#### Step 2: Update Server Configuration (`.env`)
Open `.env` in the root of your `hroot` deployment on the server and adjust the update path variables:

```env
# 1. Target Docker image repository and tag
HROOT_IMAGE=ghcr.io/<your-account>/hroot:latest

# 2. GitHub repository checked for new commits & version comparison
UPDATE_GITHUB_REPO=<your-account>/hroot

# 3. Branch tracked for updates (e.g. master, staging, or custom branch)
UPDATE_BRANCH=master
```

> [!NOTE]
> * `HROOT_IMAGE`: Tells `docker-compose.yml` which container image to download and run for the `web` and `cron` services.
> * `UPDATE_GITHUB_REPO` and `UPDATE_BRANCH`: Tell the in-app version checker (`/admin/options/check`) which GitHub repository and branch API to query when checking if a newer commit is available.

#### Step 3: Create a Pre-Switch Safety Database Backup
Before transitioning to the new image, always create an explicit database backup:

```bash
docker compose exec db mysqldump -u hroot -p${DATABASE_PASSWORD} ${DATABASE_NAME:-hroot_production} > backup_pre_switch_$(date +%F).sql
```

#### Step 4: Pull New Image and Recreate Services
Execute the transition by pulling the newly configured image and restarting the stack:

```bash
# 1. Pull the new image from your fork
docker compose pull

# 2. Recreate containers with the new image and reloaded .env configuration
docker compose up -d
```

> [!TIP]
> On container startup, the entrypoint script automatically runs `bin/rails db:prepare` (`db:migrate`), applying any new database migrations introduced in your custom fork.

#### Step 5: Verify the Switched Update Path in the Admin UI
1. Sign in as a **Banking Administrator**.
2. Go to **Admin $\rightarrow$ Options $\rightarrow$ System Check & Updates** (`/admin/options/check`).
3. Verify that:
   * The **GitHub Repository** now links to `https://github.com/<your-account>/hroot`.
   * The **Tracked Branch** shows your configured branch (`master`).
   * The **Commit Comparison** compares your running container against the latest commit in your custom fork.

---

### Switching Back to Upstream or Between Branches
To switch back to the official upstream repository or to a different branch at any time:
1. Edit `.env` to restore:
   ```env
   HROOT_IMAGE=ghcr.io/wiso-forschungslabor/hroot:latest
   UPDATE_GITHUB_REPO=wiso-forschungslabor/hroot
   UPDATE_BRANCH=master
   ```
2. Run `docker compose pull && docker compose up -d`.
3. Check `/admin/options/check` to confirm the upstream update path is active again.
