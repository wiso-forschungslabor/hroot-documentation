...
2. **Run Database Schema Migrations:**
   You can apply migrations using either the command line or the In-Browser Setup Assistant:
   * **Option A (CLI):**
     ```bash
     RAILS_ENV=production bin/rails db:migrate
     ```
   * **Option B (In-Browser Setup Assistant):**
     Open `https://<APP_DOMAIN>/setup` with your `HROOT_SETUP_TOKEN`, select **„Upgrade from v3.x to v4.0“**, verify connected database and administrators, and click **„Run v4 Upgrade & Complete Setup“**.
   *(This applies all new schema additions in v4 like survey completion links, push notifications, staff roles, updated indexes, and dynamic settings while preserving all research data).*
